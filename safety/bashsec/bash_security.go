...
func ReviewBashSecurityWithKnownVariables(command string, variables map[string]string) ReviewResult {
	...
	commands := parsedCommands(astResult)
	var review ReviewResult
	switch astResult.Kind {
	case bashast.Simple:
		review = reviewFromAST(command, astResult, embeddedScripts)
	case bashast.TooComplex:
		if bashast.IsHardBlockReason(astResult.Reason) {
			review = blockReview(astResult.Reason)
			break
		}
	...
	case bashast.ParseUnavailable:
		review = reviewBashSecurityLegacy(command)
	default:
		review = ReviewResult{
			Decision:    ReviewRequiresApproval,
...
// Review keeps the policy-facing API compact while retaining the complete
// platform Bash security review.
func Review(command string, variables map[string]string) ReviewResult {
	return ReviewBashSecurityWithKnownVariables(command, variables)
}

func parsedCommands(result bashast.ParseResult) [][]string {
	commands := make([][]string, 0, len(result.Commands))
	for _, command := range result.Commands {
		if len(command.Argv) == 0 {
			continue
		}
		commands = append(commands, append([]string(nil), command.Argv...))
	}
	return commands
}
