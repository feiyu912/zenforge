...
func TestParseForSecurityReportsCommandStructure(t *testing.T) {
	tests := []struct {
		command      string
		control      bool
		redirection  bool
		...
		{command: `echo 'a|b'`},
		{command: `echo hi | cat`, control: true},
		{command: `echo hi > out.txt`, redirection: true},
		{command: `echo $(date)`, substitution: true},
		{command: `echo hi;`, control: true},
	}
	for _, tt := range tests {
		result := ParseForSecurity(tt.command)
		if result.Kind != Simple {
			t.Fatalf("ParseForSecurity(%q) = %#v, want simple", tt.command, result)
		}
		if result.HasControlStructure != tt.control || result.HasRedirection != tt.redirection || result.HasCommandSubstitution != tt.substitution {
			t.Fatalf("ParseForSecurity(%q) structure = %#v", tt.command, result)
		}
	}
}
