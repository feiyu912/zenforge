...
| `session/prompt` | served | Send a turn into a session. An `image` part travels inline in the prompt and is delivered to the run the prompt starts; a `file` part is resolved from the store and delivered as the reference host’s own model-visible handle text, naming a read-only copy the agent’s file tools can read; an attachment whose header this host cannot measure, a receipt this session never staged, and an attachment into a run that is already answering are each refused by name (ADRs 0138, 0139). Under the plan-execute preset a question is answered in the plan stage and never reaches an execute or summary stage (ADR 0107). Every turn the host starts records the prompt's `requestId` as the run's `PromptID`, and the projected message publishes it as `source.rpcId`, which is what retires the console's local submission echo; a queued turn's text is projected from `request.steer`'s `message` (ADR 0111). |
...
The attachment store, which used to be item 3 of this list, is closed as well (ADR 0138,
ADR 0139): a prompt's image and file parts are submitted, the upload routes exist, and a
file is delivered to the model the way upstream delivers it.
...
3. **An `@`-mention parser and expander**, so a picked conversation reference reaches the
