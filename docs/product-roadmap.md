...
| MVP | Repository-scoped acceptance is implemented and test-mapped. The complete `examples/harness-agent` CLI app, loopback-only `examples/http-harness-agent` service, and independent `integration/consumer` module cover environment-owned models, Agent Skill progressive disclosure, typed tools, HITL, Docker-backed shell execution, and detached HTTP lifecycle assembly. The three scenario examples -- `examples/qa-agent`, `examples/long-task-agent`, `examples/coding-agent` -- are run end to end by the acceptance suite against a scripted OpenAI-compatible endpoint, so the real provider adapter, agent loop, tools, approval broker, skill loader and checkpoint store are exercised without a credential (ADR 0142). Live provider credentials remain an application-owned smoke test. |
...
Remote registries, install/update flows, signatures, tenancy, entitlements, and
ZenMind marketplace UX/APIs remain application or platform roadmap work.
...
- canonical single-process detached HTTP start/resume/status/attach/cancel,
...
- external acceptance, including current platform re-integration/deployment and
  a production Container Hub deployment smoke. The adapter's opt-in test now
  covers a disposable live Hub session.
