...
| detached HTTP start/resume/status/list/attach/cancel/steer contracts and error mappings work | `server/harnesshttp.TestDetachedStartAuthorizationMetaAndStatus`, `server/harnesshttp.TestDetachedRunsListsAuthorizedManagerSnapshots`, `server/harnesshttp.TestDetachedResumeAndAttachContinuity`, `server/harnesshttp.TestDetachedValidationAndErrorMappings`, `server/harnesshttp.TestDetachedHandlersRejectUnsupportedMethods`, `server/harnesshttp.TestRunManagerSteerRoutesToActiveAgentOnly` |
| queued steer messages preserve tool-result ordering before the next model turn | `harness.TestRunControllerQueuesFIFOAndRejectsClosedRuns`, `harness.TestRunnerDrainsSteersAfterToolsBeforeTheNextModelTurn` |
