...
| runnable loopback HTTP example assembles environment provider, durable stores, skills, Docker shell, HITL, and detached routes, while rejecting accidental non-loopback binding | `examples.TestHTTPHarnessExampleWiresDurableLocalService`, `examples.TestHTTPHarnessExampleRefusesNonLoopbackAddress`, `examples/http-harness-agent.TestHTTPHarnessServesDetachedRunWithCompatibleProvider`, `go test ./examples/...` |
