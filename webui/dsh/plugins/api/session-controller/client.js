			...
			*/
			...
			observeSubmissionEvent(event) {
				...
				if (event.type === "agent/inbox/spliced") {
					...
					if (Array.isArray(splice?.inserted)) for (const message of splice.inserted) this.observeSubmissionEvent({
						type: "user/message",
						data: message
					});
					return;
			...
			observeSubmissionInbox() {
				...
				for (const message of [...inbox["next-turn"], ...inbox["next-step"]]) {
					...
					if (source.kind === "user" && "rpcId" in source) this.scheduleObservedRetirement(source.rpcId, attachmentRefsIn(message.content));
				}
			}
			/**
		...
		var SessionManager = class {
			...
			*/
			handleSessionAdded(summary) {
				...
				if (summary.parentSessionId !== void 0 && this.openCatalogs.has(summary.parentSessionId)) this.scheduleCatalogRefresh(summary.parentSessionId);
			}
			/**
			...
			*/
			handleConnected() {
				...
				for (const parentSessionId of parents) this.refreshSubagents(parentSessionId);
			}
		...
		var ClientSessions = class {
			...
			*/
			...
			retainScope(id, source) {
				...
				if (this.list.getSnapshot().byId[id] === void 0) this.projectList();
				this.publishRetention(id);
				return reference;
			}
			...
			projectList() {
				...
				for (const [parentId, catalog] of Object.entries(subagentsByParent)) for (const child of catalog.entries) {
