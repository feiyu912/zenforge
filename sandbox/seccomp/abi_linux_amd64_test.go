...
func TestHostSyscallTableIsTheKernelABI(t *testing.T) {
	checkHostSyscallTable(t, archAMD64, map[string]int{
...
func TestHostAuditArchIsTheKernelABI(t *testing.T) {
	if archAMD64.AuditArch != unix.AUDIT_ARCH_X86_64 {
		t.Fatalf("amd64 audit arch = %#x, but the headers say %#x", archAMD64.AuditArch, unix.AUDIT_ARCH_X86_64)
	}
}
